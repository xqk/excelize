...
func TestAddPicture(t *testing.T) {
	...
	assert.NoError(t, f.AddPicture("Sheet1", "F21", filepath.Join("test", "images", "excel.jpg"),
		&GraphicOptions{OffsetX: 10, OffsetY: 10, Hyperlink: "https://github.com/xqk/excelize", HyperlinkType: "External", Positioning: "oneCell"}))
...
func TestAddPicture2(t *testing.T) {
	f := NewFile()
	...
	// 表头
	f.SetCellValue(sheet, "A1", "美学生活空间预算单")
	f.SetCellValue(sheet, "A2", "客户：")
	f.SetCellValue(sheet, "F2", "顾问：")
	f.SetCellValue(sheet, "A3", "联系人：")
	f.SetCellValue(sheet, "F3", "联系人：")
	f.SetCellValue(sheet, "A4", "电话：")
	f.SetCellValue(sheet, "F4", "电话：")
	f.SetCellValue(sheet, "A5", "时间：")
	f.SetCellValue(sheet, "F5", "时间：")
	f.SetCellValue(sheet, "A6", "收货地址：")
	f.MergeCell(sheet, "A1", "K1")
	f.MergeCell(sheet, "B2", "E2")
	f.MergeCell(sheet, "G2", "K2")
	f.MergeCell(sheet, "B3", "E3")
	f.MergeCell(sheet, "G3", "K3")
	f.MergeCell(sheet, "B4", "E4")
	f.MergeCell(sheet, "G4", "K4")
	f.MergeCell(sheet, "B5", "E5")
	f.MergeCell(sheet, "G5", "K5")
	f.MergeCell(sheet, "B6", "K6")
	...
	for idx, row := range [][]interface{}{
		{"空间", "位置", "图片", "产品系列", "产品名称", "规格", "数量", "单位", "单价", "金额/元", "备注"},
	} {
		cell, err := CoordinatesToCellName(1, idx+dataHeaderIdx)
		if err != nil {
			t.Error(err)
			return
		}
	...
	// 数据
	var dataStartIdx = dataHeaderIdx + 1
	images := []string{
		"4108bac67cefca1749741c0d6beb5d1cc722dec6.jpg",
		"bc503876f6be08c1bfe23c8b8811c6dc3b5de7eb.jpg",
	}
	for i, v := range images {
		coverPath := filepath.Join("test", "images", v)
		f.SetRowHeight(sheet, i+dataStartIdx, 44)
		if err := f.AddPicture(sheet, fmt.Sprintf("C%d", i+dataStartIdx), coverPath, &GraphicOptions{
			PrintObject:         &enable,
	...
	fmt.Println(f.getColWidth(sheet, 3))
...
func TestGetCellImages(t *testing.T) {
	...
	f.Pkg.Store(defaultXMLRdRichValueWebImagePartRels, []byte(fmt.Sprintf(`<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="%s" Target="https://github.com/xqk/excelize" TargetMode="External"/><Relationship Id="rId2" Type="%s" Target="../media/image1.png"/></Relationships>`, SourceRelationshipHyperLink, SourceRelationshipImage)))
	pics, err = f.GetPictures("Sheet1", "A1")
	assert.NoError(t, err)
	assert.Equal(t, 1, len(pics))
