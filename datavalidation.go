	...
	for i, ext := range decodeExtLst.Ext {
		if ext.URI == ExtURIDataValidations {
			...
			if x14DataValidations.Count == 0 {
				...
				copy(decodeExtLst.Ext[i:], decodeExtLst.Ext[i+1:])
				decodeExtLst.Ext[len(decodeExtLst.Ext)-1] = nil // 显式释放指针引用，防止内存泄露
				decodeExtLst.Ext = decodeExtLst.Ext[:len(decodeExtLst.Ext)-1]
			}
		}
	}
