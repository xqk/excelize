...
func (f *File) SetCustomProps(prop CustomProperty) error {
	...
	if idx != -1 && prop.Value == nil {
		...
		customProps.Property = append(customProps.Property[:idx], customProps.Property[idx+1:]...)
	}
	if prop.Value != nil {
...
func (p *decodeProperty) getCustomProps() (interface{}, error) {
	...
	numFields := s.NumField() // 获取字段总数

	for i := 0; i < numFields; i++ { // 改为标准的 C 风格循环
		if 11 <= i && i <= 20 && !s.Field(i).IsNil() {
			return int32(s.Field(i).Elem().Int()), nil
		}
		if 21 <= i && i <= 22 && !s.Field(i).IsNil() {
			return s.Field(i).Elem().Float(), nil
		}
		if p.Bool != nil {
			return *p.Bool, nil
		}
