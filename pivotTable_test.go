...
func TestPivotTable(t *testing.T) {
	...
	f, err = OpenFile(filepath.Join("test", "TestAddPivotTable1.xlsx"))
	assert.NoError(t, err)
	// 使用 fmt.Sprintf 格式化并强制转换为 []byte
	content := []byte(fmt.Sprintf(`<pivotCacheDefinition  xmlns="%s"><cacheSource type="external" connectionId="1"/></pivotCacheDefinition>`, NameSpaceSpreadSheet.Value))
	// 存储到 sync.Map 中
	f.Pkg.Store("xl/pivotCache/pivotCacheDefinition1.xml", content)
	_, err = f.GetPivotTables("Sheet1")
