...
func setPtrFields(immutable, mutable reflect.Value) {
	// 获取字段总数
	numFields := immutable.NumField()

	// Go 1.17 必须使用传统 for 循环
	for i := 0; i < numFields; i++ {
		srcField := immutable.Type().Field(i)
		dstField := mutable.FieldByName(srcField.Name)

		// 获取当前源字段的值
		srcValue := immutable.Field(i)

		// 检查目标字段是否有效、可设置且类型匹配
		if dstField.IsValid() && dstField.CanSet() && dstField.Type() == srcValue.Type() {
...
func countUTF16String(s string) int {
	...
	for _, r := range s {
		// 在 Go 1.17 中手动判断码点范围
		if r >= 0x10000 {
			cnt += 2 // 增补平面字符，占用 2 个 UTF-16 code units
		} else {
			cnt += 1 // BMP 字符，占用 1 个 UTF-16 code unit
		}
	}
	return cnt
}
...
func truncateUTF16Units(s string, length int) string {
	var cnt int
	for i, r := range s {
		// 手动替代 utf16.RuneLen(r)
		runelen := 1
		if r >= 0x10000 {
			runelen = 2
		}

		if cnt+runelen > length {
			return s[:i]
		}
		cnt += runelen
	}
