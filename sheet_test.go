...
func TestSheetDimension(t *testing.T) {
	...
	assert.True(t, ok)
	data1 := []byte(fmt.Sprintf(`<worksheet xmlns="%s"><sheetData/></worksheet>`, NameSpaceSpreadSheet.Value))
	assert.NoError(t, os.WriteFile(tempFile.(string), data1, 0644))
	dimension, err = f.GetSheetDimension("Sheet1")
	assert.NoError(t, err)
	assert.Empty(t, dimension)

	// Test get the worksheet dimension in stream mode without sheetData element
	data2 := []byte(fmt.Sprintf(`<worksheet xmlns="%s"></worksheet>`, NameSpaceSpreadSheet.Value))
	assert.NoError(t, os.WriteFile(tempFile.(string), data2, 0644))
	dimension, err = f.GetSheetDimension("Sheet1")
	assert.NoError(t, err)
	assert.Empty(t, dimension)
	assert.NoError(t, f.Close())
