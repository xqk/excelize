...
func (f *File) getCellStringFunc(sheet, cell string, fn func(x *xlsxWorksheet, c *xlsxC) (string, bool, error)) (string, error) {
	...
	// 步骤1：使用 sort.Search 进行二分查找，找到第一个满足条件的索引
	idx := sort.Search(len(ws.SheetData.Row), func(i int) bool {
		// 注意：sort.Search 的判断逻辑与 sort.Find 不同，这里需要调整为：
		// 返回 true 表示当前元素的 R 大于等于目标 row（这是二分查找的关键条件）
		return ws.SheetData.Row[i].R >= row
	})

	// 步骤2：判断是否找到匹配的元素（即该索引的元素 R 等于 row）
	found := false
	// 先判断索引是否在有效范围内，再判断值是否匹配，避免数组越界
	if idx < len(ws.SheetData.Row) && ws.SheetData.Row[idx].R == row {
		found = true
	} else {
		// 如果没找到，sort.Find 会返回插入位置，这里保持 idx 不变即可（与原逻辑一致）
		found = false
	}
	if !found {
		return "", nil
